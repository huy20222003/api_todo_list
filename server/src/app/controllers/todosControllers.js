...
class todosControllers {
  ...
  async create(req, res) {
    ...
    if (!name || !label) {
      res
        .status(500)
        .json({ status: false, message: 'Missing name or status!' });
    }
    ...
    try {
      ...
      res
        .status(200)
        .json({ status: true, message: 'Add todo successful!', newTodo });
    } catch (error) {
      res.status(500).json({ status: false, message: error });
    }
  }
  ...
  async edit(req, res) {
    ...
    if (!name || !label) {
      res
        .status(400)
        .json({ status: false, message: 'Missing name or label!' });
    }
  ...
  async search(req, res) {
    ...
    try {
      ...
      if (!todo) {
        ...
        res
          .status(201)
          .json({ status: true, message: 'Search successful!', todo });
      }
    } catch (error) {
      res.status(500).json({ status: false, message: error });
    }
  ...
  async filter(req, res) {
    ...
    try {
      ...
      if (!todo) {
        ...
        res
          .status(201)
          .json({ status: true, message: 'Filter successful!', todo });
      }
    } catch (error) {
      res.status(500).json({ status: false, message: error });
    }
