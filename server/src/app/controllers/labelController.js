...
class todosControllers {
    async all(req, res) {
        ...
        try {
            ...
            res.status(200).json({ status: true, message: 'Get Successful!', labels });
        } catch (error) {
    ...
    async create(req, res) {
        ...
        try {
            ...
            await newLabel.save();
            res.status(200).json({ status: true, message: 'Add todo successful!', newLabel });
        } catch (error) {
            res.status(500).json({ status: false, message: error });
        }
    }
    ...
    async search(req, res) {
        ...
        try {
            const label = await Labels.find({userId: req._id, name: { $regex: name, $options: 'i' }});
            if(!label) {
                res.status(400).json({ status: true, message: 'Search failed!' });
            } else {
                res.status(201).json({ status: true, message: 'Search successful!', label });
            }
        } catch (error) {
            res.status(500).json({ status: false, message: error });
        }
